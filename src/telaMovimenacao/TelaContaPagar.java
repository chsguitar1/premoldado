 ...
 * @author cristiano
 */
public class TelaContaPagar extends TelaConta implements InternalFrameListener {

    Contapagar contaPagaPer = new Contapagar();
    ...
    private void obterCampos() {
        contaPagaPer.setConpagdata(acaDataConta.getValor());
        contaPagaPer.setConpagnum(acaTextfieldDescr.getTexto());
        if (tipoOperacao == ALTERANDO || tipoOperacao == PESQUISANDO) {
            contaPagaPer.setConpagcod(acaTextfieldCodigo.getInteiro());
        }
        
        contaPagaPer.setConpagstatus(acaComboStatus.getValor());
        contaPagaPer.setConpagvlr(Double.parseDouble(acaMonetarioVlr.getValor()));
    }

    @Override
    protected boolean incluirBD() {
        obterCampos();
        Conexao.executaSql(contaPagaPer.getInsereSqlAvulsa());
        limparCampos();
        habilitaCampos(false);
        return true;
    }
    ...
    protected void consultar() {
        if (telaconsulta == null) {
            telaconsulta = new TelaConsCtaPag(this);
    ...
    public void preencheCampos() {
        ...
        acaTextfieldCodigo.setText(String.valueOf(contaPagaPer.getConpagcod()));
        acaTextfieldDescr.setText(contaPagaPer.getConpagnum());
        // acaDataConta.SetValor(contaPagaPer.getConrecdata());
        acaMonetarioVlr.setValor(contaPagaPer.getConpagvlr());
        acaComboStatus.setValor(contaPagaPer.getConpagstatus());
    ...
    public void consultarDados(int codigo) {
        ...
        try {
            contaPagaPer.setConpagcod(codigo);
            rs = Conexao.executaQuery(contaPagaPer.getConsultaSqlCod());
            rs.next();

            contaPagaPer.setConpagcod(rs.getInt(1));
            acaDataConta.SetValor(rs.getDate(2));
            //contaPagaPer.setConrecdata(rs.getString(2));
            contaPagaPer.setConpagvlr(rs.getDouble(3));
            contaPagaPer.setConpagnum(rs.getString(4));
            contaPagaPer.setConpagstatus(rs.getInt(5));
    ...
    protected boolean alterarBD() {
        obterCampos();
        Conexao.executaSql(contaPagaPer.getAtualizaSql());
        limparCampos();
        habilitaCampos(false);
        return true;
    }
    ...
    protected boolean excluirBD() {
        obterCampos();
        ;
        if (Conexao.executaSqlExcluir(contaPagaPer.getExcluiSql()) == Conexao.DEPENDENCIA) {
            ...
            if (opcao == JOptionPane.YES_OPTION) {
                Conexao.executaSql("EXECUTE PROCEDURE INATIVASTATUS(" + contaPagaPer.getConpagcod() + ",'contapagar','conpagcod','conpagstatus')");
                requestFocus();
            }
            limparCampos();
            return true;
        } else {
            Conexao.executaSqlExcluir(contaPagaPer.getExcluiSql());
            limparCampos();
  ...
  @Override
    protected void ajuda() {
        PlayMovie videos = new PlayMovie("contapagar.wmv");
